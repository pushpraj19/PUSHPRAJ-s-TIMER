(() => {
  ...
  async function finish(session) { user = session?.user || null; if (!user) { loginRedirect(); return; } await loadAndMerge(); window.dispatchEvent(new CustomEvent('focus-account-ready', { detail: { user } })); }

(() => {
  const note = document.querySelector('#note'); let otpEmail = '';
  ...
  byId('email-action').onclick = async () => {
    ...
    if (!otpEmail) {
      const email = byId('email').value.trim(); if (!/^\S+@\S+\.\S+$/.test(email)) { note.textContent = 'Enter a valid email address.'; return; }
      byId('email-action').disabled = true; byId('email-action').textContent = 'Sending OTP…';
      const { error } = await client.auth.signInWithOtp({ email }); byId('email-action').disabled = false;
    ...
    const token = byId('code').value.trim(); if (token.length < 6) { note.textContent = 'Enter the OTP from your email.'; return; }
